...
// Create a keyframe at the specified time with the given layer name and position
function createKeyframe(timeOfDay, layerName, comp, position, callback) {
    ...
    if (/^[0-9]+$/.test(layerName)) {
        if (activeComp && activeComp instanceof CompItem) {
            ...
            $.writeln("Found layer: " + layerName);

            var timeInSeconds = timeOfDay;
            var X = 200;
            var Y = parseInt(position.slice(1));
            Y = (60 * Y) + 365;
            $.writeln("Y: " + Y);

            var transform = layer.property("Transform");
            if (!transform) {
                $.writeln("❌ Transform property missing on layer: " + layerName);
                return;
            }

            var prop = transform.property("Position");
            if (!prop) {
                $.writeln("❌ Position property missing on layer: " + layerName);
                return;
            }

            prop.setValueAtTime(timeInSeconds, [X, Y]);
            $.writeln(layerName + "Keyframe set at time: " + timeInSeconds + " with position: [" + X + ", " + Y + "]");

            if (typeof callback === "function") {
                callback();
            }
        } else {
            $.writeln("❌ No active composition or wrong type.");
        }
    } else {
        $.writeln("❌ Layer name is not numeric: " + layerName);
    }
}


//------------------------  Load json data ----------------------------------------------
function loadJSON(jsonData, adjustedTOD, callback) {
    var loopCount = 1;
        ...
        if (jsonData.hasOwnProperty(lap)) {
            ...
            for (var i = 0; i < lapData.length; i++) {
                ...
                if (driver.hasOwnProperty("Assumed_Positions")) {
                    ...
                    var timeOfDay = timeOfDay - adjustedTOD
